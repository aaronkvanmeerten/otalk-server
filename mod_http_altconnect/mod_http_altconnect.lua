...
local function GET_xml(event)
	...
	if host_modules["bosh"] then
		xrd:tag("Link", { rel="urn:xmpp:alt-connections:xbosh", href = module:http_url("bosh", "/http-bind") }):up();
	end
	if host_modules["websocket"] then
		xrd:tag("Link", { rel="urn:xmpp:alt-connections:websocket", href = module:http_url("bosh", "/xmpp-websocket"):gsub("^http", "ws") }):up();
	end
...
local function GET_json(event)
	...
	local jrd = { links = { } };
	if host_modules["bosh"] then
		jrd.links[#jrd.links+1] = { rel="urn:xmpp:alt-connections:xbosh", href = module:http_url("bosh", "/http-bind") };
	end
	if host_modules["websocket"] then
		jrd.links[#jrd.links+1] = { rel="urn:xmpp:alt-connections:websocket", href = module:http_url("bosh", "/xmpp-websocket"):gsub("^http", "ws") }
	end
